...
export class Burger {
  constructor() {
    ...
    this._vpMobile = window.matchMedia('(max-width:767px)');
  ...
  init() {
    ...
    const breakpointChecker = () => {
      if (this._vpMobile.matches) {
        this._burger.addEventListener('click', this._onBurgerClick);
      } else {
        this._burger.removeEventListener('click', this._onBurgerClick);
        if (this._isMenuOpen) {
          this._closeMenu();
        }
      }
    };
    this._vpMobile.addListener(breakpointChecker);
    breakpointChecker();
  }
